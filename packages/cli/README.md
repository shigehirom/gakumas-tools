...
*   **引数**:
    ...
    *   `idolName`: 特定のアイドルのみ計算する場合。カンマ区切りで複数指定可能。`all` を指定すると全アイドルを順次計算します。複数指定時や後述の `--triple` オプション使用時は、指定された全アイドルのメモリーを混合したプールから最適な組み合わせを探索します。
...
*   **オプション**:
    *   `--triple`: メモリーを3つ（メイン + サブ1 + サブ2）使用する形式でシミュレーションを行います。ライブツアーイベントなどの3枠編成に対応しています。
