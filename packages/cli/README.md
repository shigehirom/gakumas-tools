...
#### 8. 未計算メモリーの抽出 (`uncached`)

指定したステージ設定において、一度もシミュレーション（メインまたはサブとして）に使用されていないメモリーをリスト表示します。

```bash
gakumas uncached <season-stage> [runs] [supportBonus]
```

*   **引数**:
    *   `season-stage`: 対象のステージ (例: `40-1`)
    *   `runs`: 判定基準とする試行回数 (デフォルト: `3000`)
    *   `supportBonus`: 判定基準とするサポートボーナス (デフォルト: `0.04`)
*   **出力**:
    *   未計算のメモリーを「プラン | アイドル名 | 楽曲名 | メモリー名」のテーブル形式で表示します。
