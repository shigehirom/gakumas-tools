...
export function registerLoadoutCommand(cli: any) {
    cli.command('loadout', 'List all saved loadouts')
        ...
        .action(async (options?: any) => {
            ...
            try {
                ...
                const child = spawn('node', ['--no-warnings', scriptPath, ...args], {
    ...
    cli.command('cleanup', 'Clean up database (loadouts and simulation_results) with support bonus lower than current setting')
        .action(async () => {
            try {
                const scriptPath = path.join(LOCAL_SCRIPTS_DIR, 'boot-cleanup-db.mjs');
    ...
    cli.command('loadout:cleanup', 'Remove loadouts with support bonus lower than current setting (Deprecated: use cleanup instead)')
        .action(async () => {
            try {
                const scriptPath = path.join(LOCAL_SCRIPTS_DIR, 'boot-cleanup-db.mjs');
                const child = spawn('node', ['--no-warnings', scriptPath], {
